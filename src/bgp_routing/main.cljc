(ns bgp_routing.main
  "Kotodama WASM entrypoint for the Bgp_routing clean-room actor (L5) — Clojure port.
  ...
  facts (`bgp_routing.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bgp_routing")
(def tier "L5")
...
(def entity-specs
  [{:entity "Message"   :plural "messages"   :id-prefix "bgprouti_mes"
    :fields [:marker :length :type]              :required [:marker :length]
    :coerce {:length :int :type :int}            :refs {}}
   {:entity "OpenMessage"      :plural "openmessages"      :id-prefix "bgprouti_ope"
    :fields [:version :myAutonomousSystem :holdTime :bgpIdentifier :optionalParametersLength] :required [:version :myAutonomousSystem]
    :coerce {:version :int :myAutonomousSystem :int :holdTime :int :optionalParametersLength :int}          :refs {}}
   {:entity "UpdateMessage"     :plural "updatemessages"     :id-prefix "bgprouti_upd"
    :fields [:withdrawnRoutesLength :totalPathAttributeLength]        :required [:withdrawnRoutesLength :totalPathAttributeLength]
    :coerce {:withdrawnRoutesLength :int :totalPathAttributeLength :int} :refs {}}
   {:entity "NotificationMessage"   :plural "notificationmessages"   :id-prefix "bgprouti_not"
    :fields [:errorCode :errorSubcode]   :required [:errorCode :errorSubcode]
    :coerce {:errorCode :int :errorSubcode :int}        :refs {}}
   {:entity "PathAttribute"    :plural "pathattributes"    :id-prefix "bgprouti_pat"
    :fields [:attributeFlags :typeCode :attributeLength :origin]          :required [:attributeFlags :typeCode]
    :coerce {:attributeFlags :int :typeCode :int :attributeLength :int :origin :int} :refs {}}
   {:entity "KeepaliveMessage"   :plural "keepalivemessages"   :id-prefix "bgprouti_kee"
    :fields [:headerOnly]   :required [:headerOnly]
    :coerce {:headerOnly :bool}        :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"bgp_routing.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bgp_routing-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bgp_routing-compat/ready)
